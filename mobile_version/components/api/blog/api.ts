...
export default class Api {
	...
	public async postPartialUpdate(id: number, data: any) {
		const result = await this.elAxios.put(
			`blog/partial-update-post/${id}`,
			data
		);
		return result;
	}
	...
	public async mainPage() {
		const result = await this.elAxios.get(`blog/get-main-page/`);
		return result;
	}

	public async userPostsGet(username: string) {
		const result = await this.elAxios.get(`blog/get-post-list/${username}`);
		return result;
	}

	public async userPostsGetActions(id: number) {
		const result = await this.elAxios.get(
			`blog/get-post-action-list/${id}`
		);
		return result;
	}

	public async userStoriesGet() {
		const result = await this.elAxios.get(`blog/get-story-list/`);
		return result;
	}

	public async storyActionGet(id: number) {
		const result = await this.elAxios.get(`blog/get-story-action/${id}`);
		return result;
	}

	public async notificationsGet() {
		const result = await this.elAxios.get(`blog/get-notifications/`);
		return result;
	}
	...
	public async postActionCreate(
		...
		const result = await this.elAxios.post(
			`blog/create-post-action/`,
			data
		);
		if (result.status != 202) {
			throw "Error!";
	...
	public async postActionDelete(id: number) {
		const result = await this.elAxios.delete(
			`blog/delete-post-action/${id}`
		);
		if (result.status !== 204) {
			throw "Error!";
		}
