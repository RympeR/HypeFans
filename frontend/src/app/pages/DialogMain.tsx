              ...
              >
                {!item?.attachments?.some((item: any) => item.file_type === 2)
                  ? CryptoJS.AES.decrypt(
                    item.text,
                    CryptoJS.enc.Utf8.parse("D?F2WNxBk_yLJhy8+Xn&2uqSSVJmN2Eh"),
                    {
...
export const DialogMain = ({ rooms }: { rooms: any }) => {
  ...
  useEffect(() => {
    ...
    wsReadClient.onopen = () => {
      ...
      wsReadClient.send(JSON.stringify({ user: uid, id: 0 }));
    };
  ...
  const sendMessage = async (messageMain: string | null) => {
    ...
    ws.send(
      JSON.stringify({
        text: !audioMessage ? encrypt(messageMain) : "",
        user: uid,
        is_payed: false,
        date: 0,
  ...
  const onScrollList = async (event: any) => {
    if (
      ...
      const response = await chatAPI.getChatMessagesPagination(
        Number(lastUrl),
    ...
    <div className="chat__dialogsMain">
      ...
      >
        <Modal.Body className="notifications__modal" style={{ padding: "0px" }}>
          <AddToChat usersList={usersInChat} invitedUsers={invitedUsers} />
        </Modal.Body>
      </Modal>
      <Modal
