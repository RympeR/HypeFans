...
export const ListsComponent = () => {
  const [lists, setLists] = React.useState({ favourites: [], last_subs: [], friends: [] });
      ...
      {currentTab === 'list' ? (
        <>
          <div
            ...
            onClick={() => (lists.favourites.length > 0 ? setCurrentTab('favourites') : null)}
          >
            <div className="notifications__listText">
              <h1>Избранные</h1>
              <h2>{lists.favourites.length} человек</h2>
            </div>
            <div>{lists?.favourites[0]?.avatar ? <img src={lists?.favourites[0]?.avatar} alt="fff" /> : null}</div>
          </div>
          <div
            className="notifications__listItem"
            onClick={() => (lists.friends.length > 0 ? setCurrentTab('friends') : null)}
          >
            <div className="notifications__listText">
              <h1>Друзья</h1>
              <h2>{lists.friends.length} человек</h2>
            </div>
            <div>{lists?.friends[0]?.avatar ? <img src={lists?.friends[0]?.avatar} alt="fff" /> : null}</div>
          </div>
          <div
            className="notifications__listItem"
            onClick={() => (lists.last_subs.length > 0 ? setCurrentTab('last_subs') : null)}
          >
            <div className="notifications__listText">
              <h1>Последние подписчики</h1>
              <h2>{lists.last_subs.length} человек</h2>
            </div>
            <div>{lists?.last_subs[0]?.avatar ? <img src={lists?.last_subs[0]?.avatar} alt="fff" /> : null}</div>
          </div>
        </>
      ) : null}
      {currentTab === 'last_subs' ? (
        <div className="notifications__walletMain">
          ...
          >
            ...
            <div style={{ marginTop: '5px', marginLeft: '8px' }}>Последние подписчики</div>
          </div>
          <div
            style={{
